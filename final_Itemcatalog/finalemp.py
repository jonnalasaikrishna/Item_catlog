...
def editemployee(role_id, emp_id):
    editedemployee = session.query(Employees).filter_by(emp_id=emp_id).one()
    ...
    if request.method == 'POST':
        ...
        return render_template('editEmployees.html', role_id=role_id,
                               emp_id=emp_id, emp=editedemployee)
# Delete items
...
def deleteemployee(role_id, emp_id):
    employee_to_delete = session.query(Employees).filter_by(
                                       emp_id=emp_id).one()
    if employee_to_delete.user_id != login_session['user_id']:
        flash("permission denied")
        return redirect(url_for('showemployees', role_id=role_id))
    if request.method == 'POST':
        ...
        return render_template('delEmployee.html', emp=employee_to_delete,
